export default class WebAudio {
    constructor(url) {
		...
		console.log("[WebAudio] Initialized with URL:", this.url);
    ...
    registerHandlers() {
		console.log("[WebAudio] Registering stream event handlers");
    ...
    start() {
		console.log("[WebAudio] start() called");
        if (!!this.connected) {
            console.warn("[WebAudio] Already connected. Skipping.");
            return;
        }

        if (!!this.audio) {
            this.audio.remove();
            console.log("[WebAudio] Removed existing audio element");
        }
    ...
    websocketDataArrived(e) {
        ...
        this.connected = true;
		console.log("[WebAudio] Packet received (", e.data.byteLength, "bytes )");
